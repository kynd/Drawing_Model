...
TestActor::TestActor(int div) {
    colorSampler.load(COMMON_COLOR_SOURCE);
    if (div == 0) {
        rects =  Illustrator::createRandomSquareGrid(ofRandom(2, 5));
    } else {
        rects =  Illustrator::createRandomSquareGrid(div);
    }
    lines[0] = createShape();
    lines[1] = createShape();
...
void TestActor::update() {
    ...
    line = PolyLineUtil::interpolatePath(lines[0], lines[1], t, true);
}

void TestActor::updateConductor(shared_ptr<ofFbo> canvas, Conductor& conductor) {
    ...
    //auto fill = shared_ptr<ShaderFill>(new NoiseGradientShaderFill(colorA, colorB, dir, center, breadth));
...
ofPolyline TestActor::createShape() {
    /*
    float minSize = 640;
