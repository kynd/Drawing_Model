...
void main() {
    ...
    vec4 nSamp = texture(noise, crd + bounds.xy + vec2(0.0, 1.0));
    vec4 nSamp2 = texture(noise, crd + bounds.xy);
    vec2 uv = crd / bounds.zw - nSamp.rg * 0.01;

    float diag = sqrt(bounds.z * bounds.z + bounds.w * bounds.w);
    vec2 sDir = vec2(dir.y, -dir.x) + nSamp.rg * 0.1;
    float t = sin(dot(uv, sDir) * diag / 100.0);

    vec2 rc = vec2(srand(crd), srand(crd + vec2(0.12, 0.34)));
    vec2 rCoord = vec2(ivec2((crd.xy + rc * 1) / 4)) * 0.5;
    vec4 samp = texture(tex, crd.xy);
    float lev =  t * 0.5 + 0.5;
    vec4 outCol = mix(colorA, colorB, smoothstep(0.0, 1.0, lev));
    //outCol = nSamp;
    //outCol.rgb += vec3(nSamp.b - nSamp2.b) * 2.0;
